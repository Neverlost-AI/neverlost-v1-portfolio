...
from build_evidence_matrix import denied_service_type, denial_context, payer_name, source_authority, source_document_type
    ...
    for index, item in enumerate(matrix, start=1):
        lines.extend(
            [
                ...
                bullet("Payer Name", item.get("payer_name")),
                bullet("Denied Service Type", item.get("denied_service_type")),
                bullet("Denial Context", item.get("denial_context")),
    ...
    for row in top_matrix_rows(matrix, 10):
        lines.extend(
            [
                ...
                bullet("Payer Name", row.get("payer_name")),
                bullet("Denied Service Type", row.get("denied_service_type")),
                bullet("Denial Context", row.get("denial_context")),
                "",
            ]
        )
    if not matrix:
...
def source_authority_guidance(source_type: str) -> tuple[str, str]:
    ...
    if source_type == "OT record":
        return (
            "Use for provider-authored OT assessment, ADL/IADL limits, self-care function, pacing, adaptive equipment, joint protection, and home-program evidence.",
            "Do not treat insurance references inside an OT note as payer decisions; keep the source authority as OT/provider functional evidence.",
        )
    if source_type == "health-system/provider record":
        return (
            "Use for provider-authored diagnoses, assessments, treatment plans, medications, referrals, clinical observations, care coordination, and follow-up needs.",
            "Do not treat disability or insurance mentions inside the record as claimant appeal evidence or payer decision evidence without a matching source.",
        )
    if source_type == "insurance denial letter":
        return (
            "Use for payer rationale, authorization barriers, medical-necessity criteria, coverage limits, appeal rights, and missing documentation questions.",
            "Do not treat clinical or therapy details mentioned in the denial as provider-authored evidence unless supported by provider/therapy records.",
        )
    return (
...
def document_extraction_statuses() -> list[dict]:
    ...
    for path in raw_files:
        ...
        sample_text = " ".join((page.get("text") or "")[:800] for page in pages[:3])
        source_type = source_document_type(path.name, sample_text)
        payer = payer_name(path.name, sample_text)
        service_type = denied_service_type(sample_text, path.name)
        status_row = {
            "file_name": path.name,
            ...
            "denial_context": denial_context(source_type, sample_text, path.name),
    ...
    for status in statuses:
        lines.extend(
            [
                ...
                bullet("Payer Name", status.get("payer_name")),
                bullet("Denied Service Type", status.get("denied_service_type")),
                bullet("Denial Context", status.get("denial_context")),
...
def build_source_authority_map() -> str:
    ...
    lines = [
        ...
        DISCLAIMER,
        "",
        "This report separates source document type, source authority, evidence content type, and denied service type so the pipeline does not mistake appeal/process evidence, provider-authored evidence, or payer-authored evidence for each other.",
        "",
        "Rule: source authority follows who authored the document. Denied service type describes what the payer decision was about. An OT record mentioning an insurance denial remains OT/provider evidence that references a payer barrier.",
        "",
    ]
    if not statuses:
